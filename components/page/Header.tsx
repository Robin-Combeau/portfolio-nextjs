  ...
  return (
    <header className="z-[999] relative">
      ...
      <nav className="flex fixed top-[0.15rem] left-1/2 h-12 -translate-x-1/2 py-2 sm:top-[1.2rem] lg:top-[1.7rem] sm:h-[initial] sm:py-0">
        <ul className="flex w-[22rem] flex-wrap items-center justify-center gap-y-1 text-base sm:text-sm lg:text-base sm:w-[initial] sm:flex-nowrap sm:gap-[0.1em] lg:gap-[0.35em] ">
          {links.map((link) =>
            <motion.li
            ...
            >
              <Link
                className="flex w-full items-center justify-center px-2 lg:px-4 py-2 lg:py-3 hover:text-[#6A8A5D] transition"
                href={link.hash}
              >
                {link.name}
              </Link>
